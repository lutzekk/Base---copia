-- CONFIGURAÇÕES
    ...
    tempo = 28000 -- tempo para dar uma volta no player (este tempo foi o que achei mais agradável visualmente, é recomendável não mudar)
    tipoaura = 813 -- número do efeito da aura (efeito de distância, pode ser identificado com /x no jogo)
...
function efeitosAura(i,tm,cid)
    if(isCreature(cid)) then
        ...
        if(i<=30 and getPlayerStorageValue(cid, aurastr)==2) then
            i = i+1
            tm = tempo/8
            return addEvent(efeitosAura,tm,i,tm,cid)
        elseif(i>30 and getPlayerStorageValue(cid, aurastr)==2) then
        else
            return TRUE
        end
    else
