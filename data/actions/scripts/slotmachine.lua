...
doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Slot Machine] Você não tem Small Coin suficiente para jogar caça-níqueis.")
return true
end
local mymoney = getPlayerItemCount(cid,coins)
doPlayerRemoveItem(cid, coins, 2) 
doPlayerSetNoMove(cid, true)
doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Slot Machine] Você empurrou a alavanca da máquina (-2 Small Coin).")
local a = item.itemid
...
addEvent(doPullBack, 500, topos)
	...
	if checkPrize({first, second, third}) == "jackpot" then
        ...
        --local win = 20
        addEvent(doSendAnimatedText, 2500, ppos, "+20", 35)
        doPlayerAddItem(cid, coins,20)
        --addEvent(setPlayerStorageValue, 20, cid, coinss, win)
        addEvent(doPlayerSendTextMessage, 2500, cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Slot Machine] Voce Ganhou 20 Small Coin.")
	...
	local prize = checkPrize({first, second, third})
        --local win = 8
        addEvent(doSendAnimatedText, 500, ppos, "+8", 35)
        
        --addEvent(setPlayerStorageValue, 8, cid, coinss, win)
        doPlayerAddItem(cid, coins,8)
        addEvent(doPlayerSendTextMessage, 500, cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Slot Machine] Voce Ganhou 8 Small Coin.")
	...
	local prize = checkPrize({first, second, third})
        --local win = 12
        addEvent(doSendAnimatedText, 500, ppos, "+12", 35)
        
        --addEvent(setPlayerStorageValue, 12, cid, coinss, win)
        doPlayerAddItem(cid, coins,12)
        addEvent(doPlayerSendTextMessage, 500, cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Slot Machine] Voce Ganhou 12 Small Coin.")
        addEvent(doPlayerSetNoMove, 500, cid, false)
	...
	local prize = checkPrize({first, second, third})
        --local win = 18
        addEvent(doSendAnimatedText, 500, ppos, "+18", 35)
        --addEvent(setPlayerStorageValue, 80, cid, coinss, win)
        doPlayerAddItem(cid, coins,18)
        addEvent(doPlayerSendTextMessage, 500, cid, MESSAGE_STATUS_CONSOLE_BLUE, "[Slot Machine] Voce Ganhou 18 Small Coin.")
        addEvent(doPlayerSetNoMove, 500, cid, false)
	end
	return true
	end
