...
local POTIONS = { 
        [13881] = {empty = 7636, splash = 2, health = {5000, 7000}}, -- Strong health potion 
} 
...
function onUse(cid, item, fromPosition, itemEx, toPosition) 
        ...
        if(not realAnimation) then 
                doCreatureSay(itemEx.uid, "Strong Healing Potion...", TALKTYPE_ORANGE_1) 
        else 
                for i, tid in ipairs(getSpectators(getCreaturePosition(cid), 1, 1)) do 
                        if(isPlayer(tid)) then 
                                doCreatureSay(itemEx.uid, "Strong Healing Potion...", TALKTYPE_ORANGE_1, false, tid) 
                        end 
                end 
        end 
