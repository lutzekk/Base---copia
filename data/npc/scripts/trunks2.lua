...
function greetCallback(cid)
    ...
    if player:getStorageValue(8000) == 35 then
        openNpcDialog(player, Npc():getId(),
            "Cell is going to organise his own budokai, we have a chance to defeat him, now go train and maybe you will save the planet.",
            "Close&Yes"
        )
    else
        openNpcDialog(player, Npc():getId(),
...
function creatureSayCallback(cid, type, msg)
    ...
    if msg == "yes" then
        if player:getStorageValue(8000) == 35 then
            player:setStorageValue(8000, 36)
