...
selfSay("Voce esta PK!", cid)
        ...
        elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) ~= 161) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Voce nao esta nessa saga", cid)
		addFocus(cid)
		
		elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) == 161) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Voce me ajuda matar o mago Babidi?", cid)
		addFocus(cid)
		
		elseif (isFocused(cid) and msg == "yes" and (getPlayerStorageValue(cid, stor) == 161)) then
		selfSay("Ok, vamos ate a nave de Babidi!", cid)
        setPlayerStorageValue(cid, stor, 162)
