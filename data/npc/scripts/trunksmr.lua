...
selfSay("Voce esta PK!", cid)
        ...
        elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) ~= 159) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Voce nao esta nessa saga", cid)
		addFocus(cid)
		
		elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) == 159) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Cell esta organizando um torneio, voce precisa vence-lo ok?", cid)
		addFocus(cid)
		
		elseif (isFocused(cid) and msg == "yes" and (getPlayerStorageValue(cid, stor) == 159)) then
		selfSay("Cuidado com Cell! Leve isso!", cid)
		doPlayerAddItem(cid,2673,5)
        setPlayerStorageValue(cid, stor, 160)
