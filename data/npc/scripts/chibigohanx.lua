...
function greetCallback(cid)
    ...
    if player:getStorageValue(8000) <= 0 then
        openNpcDialog(player, Npc():getId(), "Hi hi! Thank you. Do you want to finish this saga now?", "Close&Yes")
    else
...
function creatureSayCallback(cid, type, msg)
    ...
    if msg:lower() == "yes" then
        if player:getStorageValue(8000) <= 0 then
            ...
            openNpcDialog(player, Npc():getId(), "Take this for saving me! You've been teleported to a new location. Good luck!", "Close")
        else
