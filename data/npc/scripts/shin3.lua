...
selfSay("Voce esta PK!", cid)
        ...
        elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) ~= 165) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Voce nao esta nessa saga", cid)
		addFocus(cid)
		
		elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) == 165) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Ahora vamos matar Majin Buu, precisa de senzu?", cid)
		addFocus(cid)
		
		elseif (isFocused(cid) and msg == "yes" and (getPlayerStorageValue(cid, stor) == 165)) then
		selfSay("Mas o que?!", cid)
