...
selfSay("Voce esta PK!", cid)
        ...
        elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) ~= 176) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Voce nao esta nessa saga", cid)
		addFocus(cid)
		
		elseif((msg == "hi") and not (isFocused(cid)) and (getPlayerStorageValue(cid, stor) == 176) and getNpcDistanceToCreature(cid) <= 2) then
		selfSay("Voce tem que sair desse local!", cid)
		addFocus(cid)
		
		elseif (isFocused(cid) and msg == "yes" and (getPlayerStorageValue(cid, stor) == 176)) then
		doPlayerAddItem(cid,2673,3)
        selfSay("Ok, pegue isso!", cid)
        doTeleportThing(cid, pos1)
		setPlayerStorageValue(cid, stor, 177)
