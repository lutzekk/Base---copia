...
function greetCallback(cid)
    ...
    if player:getStorageValue(8000) == 28 then
        openNpcDialog(player, Npc():getId(),
            "Hey! My name is Trunks. I'm from the future.\nTwo androids will attack the Earth. You have to defeat them!",
            "Close&Yes"
        )
    else
        openNpcDialog(player, Npc():getId(),
...
function creatureSayCallback(cid, type, msg)
    ...
    if msg == "yes" then
        if player:getStorageValue(8000) == 28 then
            player:setStorageValue(8000, 29)
