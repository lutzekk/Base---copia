...
function creatureSayCallback(cid, type, msg)
		...
		if task_sys[task] then
			if getPlayerStorageValue(cid, task_sys[task].start) <= 0 then
				if getPlayerLevel(cid) >= task_sys[task].level then
					...
					npcHandler:say("[Task System] Congratulations, you are now participating in the Task of "..task_sys[task].name.." and must kill "..task_sys[task].count.." from this list: "..getMonsterFromList(task_sys[task].monsters_list)..". "..(#task_sys[task].items > 0 and "oh and please bring me "..getItemsFromList(task_sys[task].items).." for me." or "").."" , cid)
				else
					npcHandler:say("Sorry, but you need to level up. "..task_sys[task].level.." to be able to participate in the Task of "..task_sys[task].name.."!", cid)
				end
			else
				npcHandler:say("Sorry, but you are currently on task "..task_sys[task].name..". You may {reward} if you have already finished.", cid)
			end
		else
	...
	elseif isInArray({"diaria","daili","daily","dayli","diario"}, msg) then
		...
		elseif daily_task[daily] and getPlayerStorageValue(cid, task_sys_storages[5]) >= daily_task[daily].count then
			npcHandler:say("Sorry, you have a task for {reward} !", cid) return true
		end
       ...
       local dtask = daily_task[r]
		npcHandler:say("[Daily Task System] Congratulations, you are now participating in the Task Daily of "..dtask.name.." and must kill "..dtask.count.." monsters from this list: "..getMonsterFromList(dtask.monsters_list).." until "..os.date("%d %B %Y %X ", getPlayerStorageValue(cid,task_sys_storages[6]))..". Good luck!" , cid)
	elseif isInArray({"receber","reward","recompensa","report","reportar","entregar","entrega"}, msg) then
		...
		if v then -- original task
			if getPlayerStorageValue(cid, v.start) > 0 then
			 ...
			 if v.exp > 0 then doPlayerAddExp(cid, v.exp) str = str.."".. (str == "" and "" or ", ") .." "..v.exp.." of exp" end
		         ...
		         if table.maxn(v.reward) > 0 then GiveRewardsTask(cid, v.reward) str = str.."".. (str == "" and "" or ", ") ..""..getItemsFromList(v.reward) end
					...
					npcHandler:say("Sorry, but you haven't finished your task. "..v.name..". I need you to kill more "..(getPlayerStorageValue(cid, task_sys_storages[3]) < 0 and v.count or -(getPlayerStorageValue(cid,task_sys_storages[3])-v.count)).." Of these terrible monsters!", cid)
				end
			end
		end
		if k then -- daily task
			if getPlayerStorageValue(cid, task_sys_storages[7]) > 0 then
				...
				if k.exp > 0 then doPlayerAddExp(cid, k.exp) rst = rst.."".. (rst == "" and "" or ", ") .." "..k.exp.." of exp" end
		         ...
		         if table.maxn(k.reward) > 0 then GiveRewardsTask(cid, k.reward) rst = rst.."".. (rst == "" and "" or ", ") ..""..getItemsFromList(k.reward) end
					...
					npcHandler:say("Sorry, but you haven't finished your daily task. "..k.name..". I need you to kill more "..(getPlayerStorageValue(cid, task_sys_storages[5]) < 0 and k.count or -(getPlayerStorageValue(cid,task_sys_storages[5])-k.count)).." These monsters!", cid)
				end
			end
		end
	elseif msg == "no" then 
		selfSay("All is well then", cid) 
