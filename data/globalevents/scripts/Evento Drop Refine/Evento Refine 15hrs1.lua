...
function onTime()
    if os.date("%A") == configuration.day then
        ...
        if item > 0 then
            doRemoveItem(item, 1)
            broadcastMessage("A [Party Zone] foi fechado. Próxima abertura sera as 21h.", MESSAGE_STATUS_WARNING)
            ...
            broadcastMessage("A [Party Zone] foi aberto e fechará as 16:00", MESSAGE_STATUS_WARNING)
        end
    end
