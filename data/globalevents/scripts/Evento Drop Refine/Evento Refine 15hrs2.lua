...
function onTime()
    if os.date("%A") == configuration.day then
        ...
        if item > 0 then
            doRemoveItem(item, 1)
            broadcastMessage("O [Evento Drop Refine] foi fechado. Próxima abertura sera as 15h.", MESSAGE_STATUS_WARNING)
            ...
            broadcastMessage("O [Evento Drop Refine] foi aberto e fechará as 10:30", MESSAGE_STATUS_WARNING)
        end
    end
	if os.date("%A") == configuration.day2 then
		...
		doRemoveItem(item, 1)
            ...
            doCreateTeleport(configuration.teleport_id, configuration.to_pos, configuration.pos)
            broadcastMessage("[Party Zone] Foi Aberta e Fechara as 16hrs", MESSAGE_STATUS_WARNING)
        end
    end
    return true
end