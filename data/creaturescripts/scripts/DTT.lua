local function endExpRate(cid)
    if isPlayer(cid) == TRUE then
        ...
        setPlayerStorageValue(cid, dtt.storage.exp_bonus, -1)
        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_RED, "[DEFEND THE TOWER] O efeito do seu bonus acabou!")
    end
end
...
function onLogin(cid)
    ...
    dtt.setBonusExp(cid)
    local str = getPlayerStorageValue(cid, dtt.storage.exp_bonus)
    if(str >= 0 and (str - os.time()) > 0) then
        local atualExp = getPlayerRates(cid)[SKILL__LEVEL]
        print("XP Antes:".. atualExp)
        doPlayerSetRate(cid, SKILL__LEVEL, atualExp+dtt.bonus_rate)
        local atualExp = getPlayerRates(cid)[SKILL__LEVEL]
        print("XP Depois do bonus dtt:".. atualExp)
        doPlayerSendTextMessage(cid, 25, "[DEFEND THE TOWER] Voce esta com "..((dtt.bonus_rate - 1)*100).."% de bonus experiencia.")
