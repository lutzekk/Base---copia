function onDeath(player, corpse, killer, mostDamageKiller, lastHitUnjustified, mostDamageUnjustified)
    ...
    if amulet and amulet.itemid == ITEM_AMULETOFLOSS and not isRedOrBlack then
        ...
        -- Lista de ítems protegidos que no se perderán
        local protectedItems = {2003, 2510} -- Ejemplo: Golden Boots (2646) y Soft Boots (6132)
        ...
        for i = CONST_SLOT_HEAD, CONST_SLOT_AMMO do
            ...
            if item then
                -- Si el ítem está en la lista de protegidos, se ignora y no se pierde
                if table.contains(protectedItems, item.itemid) then
                    goto continue
                end
