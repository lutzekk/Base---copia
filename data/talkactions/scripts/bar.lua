...
function onSay(cid, words, param)
    ...
    if not t[1] then
        ...
        if bars ~= "" then
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Suas barras dispon�veis s�o: " .. string.sub(bars, 1, -3) .. ".")
        else
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voc� n�o possui nenhuma aura dispon�vel.")
        end
        return true
    end
    ...
    if bar then
        if getPlayerStorageValue(cid, bar.storage) >= 1 then
            ...
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voc� n�o possui essa barra. Para adquiri-la, acesse a loja.")
        end
    else
        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Comando inv�lido.")
    end
    return true
end