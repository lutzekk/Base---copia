...
PASS_ITEMS = {
	...
	[10] =
	{
		items = {2160, 2152, 7964, 7965, 7966}, -- lista de items
		count = {10, 15, 1, 1, 1}, -- quantidade de items
		level = {1, 2, 3, 4,5}, -- nivel necessario para resgatar

		types = {"Basic", "Elite", "Basic", "Basic", "Elite"}, -- ranking necessario para desbloquear a recompensa
		storagesUnlock = {4922, 4923, 4924, 4925, 4926}, -- storages usadas para desbloquear o item
	},
...
-- lembrando que as miss�es n�o resetam, ent�o voc� tem que adicionar muitas, ou at� mesmo ficar mudando a storage.
	[1] = {name = "Wolf LvL 3", type = "kill", storage = 8894, completed_storage = 9998, count = 1, points = 50, image = 26, text = "Derrote 50 Snake"},
	[2] = {name = "Wolf LvL 3", type = "kill", storage = 8895, completed_storage = 9999, count = 1, points = 50, image = 26, text = "Derrote 100 Snake"},
	[3] = {name = "Wolf LvL 3", type = "kill", storage = 8896, completed_storage = 10000, count = 2, points = 50, image = 26, text = "Derrote 150 Snake"},
	[4] = {name = "Wolf LvL 3", type = "kill", storage = 8897, completed_storage = 10001, count = 3, points = 50, image = 58, text = "Derrote 300 Soldier"},
	[5] = {name = "Wolf LvL 3", type = "kill", storage = 8898, completed_storage = 10002, count = 4, points = 50, image = 58, text = "Derrote 500 Soldier"},
	[6] = {name = "Wolf LvL 3", type = "kill", storage = 8899, completed_storage = 10003, count = 5, points = 50, image = 58, text = "Derrote 1000 Soldier"},
	[7] = {name = "Wolf LvL 3", type = "kill", storage = 8900, completed_storage = 10004, count = 6, points = 50, image = 36, text = "Derrote 1000 Super Namekjin"},
	[8] = {name = "Wolf LvL 3", type = "kill", storage = 8901, completed_storage = 10005, count = 1500, points = 50, image = 36, text = "Derrote 1500 Super Namekjin"},
	[9] = {name = "Wolf LvL 3", type = "kill", storage = 8902, completed_storage = 10006, count = 2000, points = 50, image = 36, text = "Derrote 2000 Super Namekjin"},
}
...
PASS_FINISH = "Acaba em: " .. PASS_ENDDAY .. "/" .. PASS_ENDWEEK .. "/" .. PASS_STARTYEAR -- lembrando que as datas precisam colidir com o mes, por exemplo: come�a no dia 01/02/2023 e acaba no dia 31/02/2023, se voce deixar por exemplo: 31/01/2023 pra come�ar e acabar em: 31/02/2023 n�o vai dar certo pq os meses s�o diferentes.
...
function collectRecompensePass(cid)
	...
	-- Verificar si tiene el pase élite o básico
	local playerPassType = getPlayerStorageValue(cid, 93929)
	if playerPassType ~= 1 then
		-- Si no tiene el pase élite, mostramos un mensaje y no le permitimos recoger recompensas élite
		doPlayerPopupFYI(cid, "No tienes el pase élite para recolectar estas recompensas.")
		return true
	end
	...
	if tabela then
		...
		for i = 1, #tabela.items do
			...
			if tabela.types[i] == "Elite" and playerPassType == 1 and getPlayerStorageValue(cid, tabela.storagesUnlock[i]) <= 0 and getPlayerPassLevel(cid) >= tabela.level[i] then
...
function buyPassElite(cid)
    ...
    if getPlayerStorageValue(cid, 939309) == 1 then
    ...
    setPlayerStorageValue(cid, 93939, 1)
