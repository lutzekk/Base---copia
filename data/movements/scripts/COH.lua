...
function onStepIn(cid, item, pos, fromPosition)
 ...
 if item.actionid == 16203 then 
  ...
  if getGlobalStorageValue(COH_STATUS) == getPlayerGuildName(cid) then
   ...
   doPlayerSendCancel(cid, "[CoH] Voce não pertence á guild "..getGlobalStorageValue(COH_STATUS)..".")
  end
  return true
 end
  ...
  if getPlayerGuildId(cid) > 0 then
   if (getGlobalStorageValue(COH_STATUS) ~= getPlayerGuildName(cid)) then
  		doPlayerSendTextMessage(cid, 20, "[Castle Of Honor] Voce e sua guild estão no comando, os antigos donos ["..tostring(getGlobalStorageValue(COH_STATUS)).."] podem se vingar!")
   ...
   doTeleportThing(cid, fromPosition, false)
   doPlayerSendCancel(cid, "[CoH] Voce não possui uma guild.")
   end
 	 return true
 end
  ...
  if getPlayerGuildId(cid) > 0 then
   ...
   doSendMagicEffect(pos, 2)  
   doTeleportThing(cid, fromPosition, false)
   doPlayerSendCancel(cid, "[Castle Of Honor] Voce não possui uma guild.")   
   return true
  end   
 end
