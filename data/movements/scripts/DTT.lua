function onStepIn(cid, item, position, fromPosition)
	...
	if item.actionid == 9802 and (getPlayerStorageValue(cid, dtt.storage.team_blue) ==  1) then
		local pos = getCreaturePosition(cid)
		local newPos = {x=pos.x+1, y=pos.y, z=pos.z}
		doTeleportThing(cid, newPos)
		return true
	end
		if item.actionid == 9806 and (getPlayerStorageValue(cid, dtt.storage.team_blue) ==  1) then
		local pos = getCreaturePosition(cid)
	...
	if item.actionid == 9803 and (getPlayerStorageValue(cid, dtt.storage.team_red) ==  1) then
		local pos = getCreaturePosition(cid)
		local newPos = {x=pos.x-1, y=pos.y, z=pos.z}
		doTeleportThing(cid, newPos)
		return true
	end
		if item.actionid == 9805 and (getPlayerStorageValue(cid, dtt.storage.team_red) ==  1) then
		local pos = getCreaturePosition(cid)
		local newPos = {x=pos.x, y=pos.y+1, z=pos.z}
		doTeleportThing(cid, newPos)
		return true
